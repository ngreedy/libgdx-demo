 ...
 * @date 2020/11/10.
 */
public class GdxAttach implements AndroidApplicationBase {
    ...
    public void setLifeOwner(@NotNull Activity ownerActivity, ViewGroup container) {
    ...
    private void init(ApplicationListener listener, AndroidApplicationConfiguration config, boolean isForView) {
        ...
        input = AndroidInputFactory.newAndroidInput(this, ownerActivity, graphics.view, config);
        audio = new AndroidAudio(ownerActivity, config);
        ownerActivity.getFilesDir(); // workaround for Android bug #10515463
        files = new AndroidFiles(ownerActivity.getAssets(), ownerActivity.getFilesDir().getAbsolutePath());
        net = new AndroidNet(this/*, config*/);
        this.listener = listener;
        this.handler = new Handler();
        this.useImmersiveMode = config.useImmersiveMode;
        this.hideStatusBar = config.hideStatusBar;
        this.clipboard = new AndroidClipboard(ownerActivity);
        ...
        if (!isForView) {
            try {
                ownerActivity.requestWindowFeature(Window.FEATURE_NO_TITLE);
            } catch (Exception ex) {
                log("AndroidApplication", "Content already displayed, cannot request FEATURE_NO_TITLE", ex);
            }
            ownerActivity.getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN);
            ownerActivity.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_FORCE_NOT_FULLSCREEN);
//            ownerActivity.setContentView();
            container.addView(graphics.getView(), createLayoutParams());
        ...
        if (ownerActivity.getResources().getConfiguration().keyboard != Configuration.KEYBOARD_NOKEYS)
    ...
    protected void createWakeLock(boolean use) {
        if (use) {
            ownerActivity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        }
    }
    ...
    protected void hideStatusBar(boolean hide) {
        ...
        View rootView = ownerActivity.getWindow().getDecorView();
    ...
    public void useImmersiveMode(boolean use) {
        ...
        View view = ownerActivity.getWindow().getDecorView();
        try {
            Method m = View.class.getMethod("setSystemUiVisibility", int.class);
...
//        graphics.onPauseGLSurfaceView();
    ...
    public Preferences getPreferences(String name) {
        return new AndroidPreferences(ownerActivity.getSharedPreferences(name, Context.MODE_PRIVATE));
    }
...
ownerActivity.startActivity(intent);
    ...
    public Window getApplicationWindow() {
        return ownerActivity.getWindow();
    }

    @Override
    public WindowManager getWindowManager() {
        return ownerActivity.getWindowManager();
    }
